@props([ 'experience', ]) @php $modalId = 'experience-' . $experience->id; $titleId = $modalId .
'-title'; @endphp
...
<dialog
  ...
  x-data="experienceModal(@js($modalId))"
  x-on:open-experience-modal.window="
...
>
  <div class="experience-modal__panel">
    <header class="experience-modal__header">
      <div>
        <p class="experience-modal__period">
          {{ $experience->period_label }} @if ($experience->employment_type) · {{
          $experience->employment_type }} @endif
        </p>

        <h2 id="{{ $titleId }}" class="experience-modal__title">{{ $experience->job_title }}</h2>

        <p class="experience-modal__company">
          {{ $experience->company_name }} @if ($experience->location) · {{ $experience->location }}
          @endif
        </p>
      </div>
    ...
    <div class="experience-modal__content">
      ...
      <p class="experience-modal__description">{{ $experience->details }}</p>
      @endif @if (! empty($experience->responsibilities))
      ...
      @endif @if (! empty($experience->achievements))
      <section class="experience-modal__section">
        <h3>Réalisations</h3>
      ...
      @endif @if (! empty($experience->technologies))
      <section class="experience-modal__section">
    ...
    <footer class="experience-modal__footer">
      ...
      <button type="button" class="btn btn-primary" x-on:click="close()">Fermer</button>
    </footer>
  </div>
</dialog>
