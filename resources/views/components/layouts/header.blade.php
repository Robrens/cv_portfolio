@props([ 'profile', 'skillCategories', 'experiences', 'workMethods', ]) @php $hasAbout =
filled($profile->about_title) || filled($profile->about_description) ||
...
mb_substr($profile->first_name, 0, 1) . mb_substr($profile->last_name, 0, 1) ); $resumeUrl =
$profile->resume_path ? asset('storage/' . $profile->resume_path) : null; @endphp
...
>
  <div class="site-header-container">
    <div class="flex items-center justify-between gap-10">
      <a href="/" class="flex shrink-0 items-center gap-3">
        <span class="title text-brand-accent"> {{ $initials }} </span>
      </a>

      <nav
        class="hidden flex-1 items-center justify-center gap-8 lg:flex"
      ...
      >
        ...
        <a
          href="{{ request()->routeIs('home') ? '#skills' : route('home') . '#skills' }}"
          class="nav-link"
        ...
        <a
          href="{{ request()->routeIs('home') ? '#career' : route('home') . '#career' }}"
          class="nav-link"
          :class="{ 'is-active': activeSection === 'career' }"
        ...
        <a
          href="{{ request()->routeIs('home') ? '#approach' : route('home') . '#approach' }}"
          class="nav-link"
          :class="{ 'is-active': activeSection === 'approach' }"
        ...
        <a
          href="{{ request()->routeIs('home') ? '#passions' : route('home') . '#passions' }}"
          class="nav-link"
          :class="{ 'is-active': activeSection === 'passions' }"
        ...
        @endif @if ($hasContact)
        <a
          href="{{ request()->routeIs('home') ? '#contact' : route('home') . '#contact' }}"
          class="nav-link"
      ...
      <a
        href="{{ $resumeUrl }}"
        class="btn btn-primary hidden shrink-0 lg:inline-flex"
  ...
  <aside
    id="mobile-menu"
    x-show="open"
    x-transition:enter="transition-transform duration-300 ease-soft"
