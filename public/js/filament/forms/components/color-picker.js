...
var at = { grad: 360 / 400, turn: 360, rad: 360 / (Math.PI * 2) },
  ...
  v = (e) => (
    ...
    e.length < 6
      ...
      : {
          ...
          a: e.length === 8 ? a(parseInt(e.substring(6, 8), 16) / 255, 2) : 1,
        }
  ),
  ...
  Y = ({ h: e, s: t, v: r, a: o }) => {
    ...
    return {
      h: a(e),
      s: a(s > 0 && s < 200 ? ((t * r) / 100 / (s <= 100 ? s : 200 - s)) * 100 : 0),
      l: a(s / 2),
      a: a(o, 2),
    };
  };
...
var I = (e) => {
  ...
  G = ({ r: e, g: t, b: r, a: o }) => {
    let s = Math.max(e, t, r),
      n = s - Math.min(e, t, r),
      i = n ? (s === e ? (t - r) / n : s === t ? 2 + (r - e) / n : 4 + (e - t) / n) : 0;
    return {
      h: a(60 * (i < 0 ? i + 6 : i)),
      s: a(s ? (n / s) * 100 : 0),
      v: a((s / 255) * 100),
...
function gt({
  ...
  return {
    ...
    init() {
      (this.state === null || this.state === '' || this.setState(this.state),
        ...
        this.$watch(
          'state',
          Alpine.debounce((l) => {
            CSS.supports('color', l) && (this.$refs.picker.color = l);
          }, 200),
        ),
        ...
        (r || o || s) &&
          new MutationObserver(() => (this.isOpen() ? null : this.commitState())).observe(
            this.$refs.panel,
    ...
    setState(l) {
      ((this.state = l), (this.$refs.input.value = l), (this.$refs.picker.color = l));
    },
    isOpen() {
    ...
    commitState() {
      JSON.stringify(this.$wire.__instance.canonical) !==
        JSON.stringify(this.$wire.__instance.ephemeral) && this.$wire.$commit();
    },
  };
}
